/*Abhishek Agrawal
20194144
  Program to implement a Header Based Circular Linked List*/
#include<stdio.h>
#include<stdlib.h>
struct node{
	int data;
...
void insert_node_before(int x,int y)
{
	...
	NODE *q = head->next;
	while(q->next->data!=y&&q->next->next!=head)
	{
		q = q->next;
	}
	if(q->next->data!=y&&q->next->next==head){
...
void delete_given_node(int x)
{
	...
	while(q->next!=head&&q->next->data!=x)
	{
		q= q->next;
	}
